...
class prepareExamine5Leader(unittest.TestCase):
    def setUp(self):
    ...
    def test_prepare_examine5_leader(self):
        driver = self.driver
        ...
        driver.find_element_by_xpath("//textarea").send_keys(u"工程验收领导审核通过！！！")
        time.sleep(2)
        driver.find_element_by_xpath("(//button[@type='button'])[9]").click()
        driver.refresh()
        driver.find_element_by_id("tab-5").click()
        time.sleep(3)
        print("工程验收领导审核成功！！！")
